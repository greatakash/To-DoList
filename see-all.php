...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.2/jquery.min.js"></script>
							...
							/* ---showing the recent added events and by click you can see the event as well.*/ ?>
								<tr>
									...
									<td><div class="controls">
											 <select class="span9 chosen" tabindex="1" id="stat<?=$cat['e_id']?>" onChange="changestatus(this.value,'<?=$cat['e_id']?>')">
												<option value="true" <?=$cat['status'] == 'Yes' ? 'selected' : ''?>>Active</option>
												<option value="false" <?=$cat['status'] == 'No' ? 'selected' : ''?>>InActive</option>
											 </select>
										  </div></td>
	...
	{
		$.post('ajax/statususer.php',{ stat : stat , id : id });
	}
	</script>
